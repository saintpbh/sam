...
def generate_pony_portrait(character_info, output_path):
    ...
    # Construct details string from character info used in previous scripts or defaults
    # For now, simplistic mapping or usage of description if available
    details = character_info.get('desc', 'ornate Han dynasty armor and silk robes, detailed face, majestic pose')
    
    positive_prompt = f"{base_positive} {character_name}, {details}, standalone character, white background, centered, dynamic 2D illustration"
    ...
    # Implemented based on inspected node inputs
    workflow = {
        "4": {
            "inputs": {
        ...
        "9": {
            "inputs": {
                "filename_prefix": "RTK_Pony_Ultimate",
                "images": ["12", 0]
            },
    ...
    print(f"Generating [PONY ULTIMATE] for {character_name}...")
    try:
    ...
    while True:
        ...
        if time.time() - start_time > 1200: # 20 min timeout for high res
            print("Timeout")
            return False
        time.sleep(5)
...
if __name__ == "__main__":
    ...
    for char_data in characters:
        # Handle list format [name, filename] or dict
    ...
    for loc in locations:
        ...
        filename = loc[1]
        desc = loc[2]
        target = os.path.join(output_dir, filename) # Saving to same dir for now or change to public/maps
        # Reuse function but strictly prompts will handle the difference
        # Ideally we'd have a separate function but Pony handles 'scenery' well too with score tags
